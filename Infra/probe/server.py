...
async def _lifespan(app: FastAPI):
    ...
    # PROBE (issue 10 / 06): the ZipEnhancer denoiser is never used by this server but is
    # eagerly built at startup, pulling iic/speech_zipenhancer…@master from modelscope.cn
    # into an unmounted ~/.cache/modelscope — fatal under read_only + offline.
    _model = VoxCPM.from_pretrained(model_id, optimize=optimize, load_denoiser=False)
