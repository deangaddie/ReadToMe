...
# PROBE (issue 10, finding 2): qwen_tts asks AutoProcessor for fix_mistral_regex=True, and
# transformers' _patch_mistral_regex calls the live Hub API (model_info) for any *repo id* —
# egress on every boot, fatal under HF_HUB_OFFLINE. Resolving the pinned revision to a local
# snapshot path first makes transformers take its _is_local branch and skip the call.
from huggingface_hub import snapshot_download

MODEL_PATH = snapshot_download(MODEL_NAME, revision=MODEL_REVISION, local_files_only=True)
