...
namespace Read2Me.Tests.Services.Audio
{
    public class ChatterboxParagraphTtsClientTests
    {
        ...
        [Fact]
        public async Task GenerateAsync_PostsMultipartToTtsEndpoint_WithTextAndReferenceAudio()
        {
            var handler = new FakeHttpMessageHandler(FakeWav());
            var factory = new FakeHttpClientFactory(handler);
            var sut = new ChatterboxParagraphTtsClient(factory, NullLogger<ChatterboxParagraphTtsClient>.Instance, new FakeAiServiceReporter());

            using var refAudio = new MemoryStream(Encoding.UTF8.GetBytes("fake-wav"));
            await sut.GenerateAsync("hello world", "ignored instructions", refAudio, Config, null);
        ...
        [Fact]
        public async Task GenerateAsync_ForwardsAllSixNumericParams_InvariantCultureFormatted()
        {
            var handler = new FakeHttpMessageHandler(FakeWav());
            var factory = new FakeHttpClientFactory(handler);
            var sut = new ChatterboxParagraphTtsClient(factory, NullLogger<ChatterboxParagraphTtsClient>.Instance, new FakeAiServiceReporter());
            ...
            Assert.Equal((0.5).ToString(CultureInfo.InvariantCulture), handler.Fields["exaggeration"]);
            Assert.Equal((0.5).ToString(CultureInfo.InvariantCulture), handler.Fields["cfg_weight"]);
            Assert.Equal((0.8).ToString(CultureInfo.InvariantCulture), handler.Fields["temperature"]);
            Assert.Equal((0.05).ToString(CultureInfo.InvariantCulture), handler.Fields["min_p"]);
            Assert.Equal((1.0).ToString(CultureInfo.InvariantCulture), handler.Fields["top_p"]);
        ...
        public async Task GenerateAsync_OnHttpFailure_ManagedService_ThrowsAiServiceUnavailableException()
        {
            ...
            var sut = new ChatterboxParagraphTtsClient(factory, NullLogger<ChatterboxParagraphTtsClient>.Instance, reporter);
