...
{
    public class SemanticSimilaritySettingsServiceTests : AppDbTestBase
    {
        private SemanticSimilaritySettingsService NewService() =>
            new(Factory, NullLogger<SemanticSimilaritySettingsService>.Instance);

        private static SemanticSimilarityServiceConfig Config(string name) => new()
        {
            Name = name,
