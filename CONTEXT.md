...
- **Audio Queue** — the background pipeline (not yet built) that generates audio for selected ParagraphItems. The user adds items via `AudioItemSelection`; the queue processes them one at a time using the Character's voice (for Character items) or the narrator voice (for Narration items).
